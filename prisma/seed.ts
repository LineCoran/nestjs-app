...
async function main() {
  ...
  const companyPayload = {
    ...
    phones: ['+7 (914) 025-33-48', '+7 (917) 596-96-70', '+7 (924) 794-71-56'],
    socialLinks: [
      { platform: 'telegram', label: 'Канал', value: 'gory_po_koleno_kamcha', url: 'https://t.me/gory_po_koleno_kamcha' },
      { platform: 'telegram', label: 'Группа', value: 'PowderFactory', url: 'https://t.me/PowderFactory' },
      { platform: 'instagram', label: 'Аккаунт', value: '@gory_po_koleno_kamcha', url: 'https://instagram.com/gory_po_koleno_kamcha' },
      { platform: 'vk', label: 'Группа', value: 'Туры по Камчатке с «Горы по колено»', url: 'https://vk.com/gorypokoleno' },
      { platform: 'whatsapp', label: 'Группа', value: 'Путешествуй с "Горы по колено"', url: 'https://wa.me/79140253348' },
    ...
    kpp: '410001001',
    ogrn: '1214100004580',
    bankName: 'СЕВЕРО-ВОСТОЧНОЕ ОТДЕЛЕНИЕ N8645 ПАО СБЕРБАНК г Магадан',
    settlementAccount: '40702810736170008576',
    correspondentAccount: '30101810300000002048',
    bic: '44442607',
    stats: [
  ...
  for (const name of blogCategoryNames) {
    ...
    blogCategories[name] = cat.id;
  }
  console.log(`✅ Категорий блога: ${blogCategoryNames.length}`);

  // ── Блог ──
  const blogData = [
    { title: 'Как подготовиться к восхождению на вулкан', excerpt: 'Снаряжение, физподготовка и что важно знать новичку перед первым восхождением.', category: 'Лайфхаки' },
    { title: 'Когда лучше ехать на Камчатку', excerpt: 'Разбираем сезоны: что доступно летом, а что — только зимой.', category: 'Лето на Камчатке' },
    { title: '5 вулканов, которые стоит увидеть', excerpt: 'От доступного Авачинского до сурового Толбачика — наш топ.', category: 'Новости' },
    { title: 'Медведи Камчатки: правила безопасности', excerpt: 'Как вести себя при встрече с косолапым и почему их не стоит бояться.', category: 'Медведи' },
    { title: 'Долина гейзеров: что нужно знать', excerpt: 'Как добраться, что увидеть и почему это одно из чудес России.', category: 'Новости' },
    { title: 'Хели-ски на Камчатке для начинающих', excerpt: 'Реально ли покататься с вертолёта, если ты не профи? Разбираемся.', category: 'Фрирайд' },
    { title: 'Что взять с собой в горный поход', excerpt: 'Полный чек-лист снаряжения от наших гидов.', category: 'Лайфхаки' },
    { title: 'Горячие источники Камчатки', excerpt: 'Лучшие термальные источники, где можно отдохнуть после маршрута.', category: 'Зимние истории' },
    { title: 'Сплав по реке Быстрая: впечатления', excerpt: 'Рассказываем о самом популярном водном маршруте полуострова.', category: 'Лето на Камчатке' },
    { title: 'Камчатская кухня: что попробовать', excerpt: 'Краб, икра, папоротник и другие гастрономические открытия.', category: 'Новости' },
  ];
  for (let i = 0; i < blogData.length; i++) {
    const b = blogData[i];
    const slug = `post-${i + 1}`;
    // Разный объём текста, чтобы время чтения отличалось между статьями.
    const paragraph =
      'Камчатка — удивительный край вулканов, гейзеров и нетронутой природы. В этой статье мы делимся опытом и практическими советами, которые пригодятся каждому, кто планирует путешествие. ';
    await prisma.blogPost.create({
      data: {
        ...
        content: `<p>${b.excerpt}</p><p>${paragraph.repeat(3 + i * 2)}</p><h2>Главное</h2><p>Готовьтесь заранее, доверяйте гидам и наслаждайтесь моментом. Остальное мы берём на себя.</p>`,
        categoryId: blogCategories[b.category],
